...
class Network:
    ...
    def __init__(self,neuron_counts,activations=[],optimizer=None,one_hot_output=True):
        ...
        if len(activations)==0: #crée une liste vide si aucune fonction d'activation est spécifiée
        ...
        for i,function in enumerate(activations):  #assigne les fonctions d'activations aux étages
            match function:
                case "relu":    #relu
        ...
        self.optimizer = util.gradient_descent  #optimisateur par défaut
    
        match optimizer:    #assigne l'optimisateur du réseau et les variables nécéssaires s'il y a lieu
        ...
        self.one_hot_output = one_hot_output    #détermine si les sorties du réseau sont encodées
    ...
    X représente les données d'entrainement du réseau 
    Y représente les réponses voulues du réseau
    ...
    def backprop(self,x,y):
        ...
        if self.one_hot_output: #convertir Y en liste utilisable par le réseau
            y = util.one_hot(y)  
    ...
    permet d'entrainer un réseau avec un échantillon x,y 
